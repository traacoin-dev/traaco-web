...
(function () {
  ...
  if (menuBtn && mobileMenu) {
    ...
    menuBtn.addEventListener('click', (e) => {
      ...
      // Reset inner scroll so items always appear centred when re-opened after scrolling
      if (isOpen && mobileMenuInner) mobileMenuInner.scrollTop = 0;
    });
  ...
  const demoBtnEl = document.getElementById('niche-demo-btn');
  const demoLabel = document.getElementById('niche-demo-label');

  // Config per filter: { label, href }
  const demoBtnConfig = {
    all:    { label: 'Look Demos',            href: 'rest.html' },
    food:   { label: 'Look Restaurant Demos', href: 'rest.html' },
    travel: { label: 'Look Travel Demos',     href: 'contact.html#contact-form' },
  };

  function updateDemoBtn(filter) {
    if (!demoBtnEl || !demoLabel) return;
    const cfg = demoBtnConfig[filter] || demoBtnConfig['all'];
    demoLabel.textContent = cfg.label;
    demoBtnEl.href        = cfg.href;
  }
